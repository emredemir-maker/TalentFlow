...
// We want to keep everything up to just before the conditional returns
// Usually around line 375 in original file (right after copyLink)
...
const newJSX = \`
    ...
    if (!isAuthenticated) return (
        <div className="min-h-screen bg-slate-50 flex items-center justify-center p-6 text-slate-900">
            <div className="text-center space-y-4">
                ...
                <h1 className="text-xl font-bold">Yetkisiz Erişim</h1>
                <p className="text-slate-500 font-medium">Lütfen giriş yapın.</p>
            </div>
        </div>
    );
    ...
    if (!candidateData) {
        return (
            <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-6 gap-4 text-slate-900">
                <AlertCircle className="w-12 h-12 text-amber-500" />
                <h1 className="text-xl font-bold">Oturum Bulunamadı</h1>
                <p className="text-slate-500 font-medium">Geçersiz veya süresi dolmuş mülakat linki.</p>
                <button
                    onClick={() => navigate('/candidates')}
                    className="px-6 py-2.5 rounded-xl bg-white border border-slate-200 text-slate-800 text-xs font-bold uppercase tracking-widest shadow-sm hover:bg-slate-50 transition-all"
                >
                    ...
                    <div className="w-[380px] lg:w-[440px] bg-slate-50 border-l border-slate-200 flex flex-col shrink-0 z-30">
                         ...
                         <div className="p-6 space-y-6 overflow-y-auto">
                              ...
                              <div className="flex-1 overflow-y-auto">
                                   <h4 className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-4">Soru İlerlemesi {currentQuestionIndex + 1}/{questions.length}</h4>
                                   <div className="space-y-3">
                                        {questions.map((q, i) => (
                                             <div key={i} className={\`p-4 rounded-xl border \${i === currentQuestionIndex ? 'bg-blue-50 border-blue-200 shadow-sm' : i < currentQuestionIndex ? 'bg-emerald-50 border-emerald-100 opacity-70' : 'bg-white border-slate-200 opacity-60'}\`}>
                                                  <div className="flex items-start gap-3">
                                                       <div className={\`w-6 h-6 rounded-full flex items-center justify-center shrink-0 \${i === currentQuestionIndex ? 'bg-[#1e3a8a] text-white' : i < currentQuestionIndex ? 'bg-emerald-500 text-white' : 'bg-slate-200 text-slate-500'}\`}>
...
function SettingsModal({ onClose, devices, selectedDevices, onDeviceChange }) {
    return (
        <div className="fixed inset-0 z-[300] bg-black/40 backdrop-blur-sm flex items-center justify-center p-6 italic">
            <div className="bg-white border border-slate-200 w-full max-w-md rounded-[2.5rem] overflow-hidden shadow-[0_20px_50px_rgba(0,0,0,0.1)] transition-all animate-in zoom-in-95 duration-200">
                <div className="p-8 space-y-8">
                    <div className="flex items-center justify-between">
                        <h3 className="text-xl font-bold text-slate-900 tracking-tight italic">Cihaz Ayarları</h3>
                        <button onClick={onClose} className="w-10 h-10 rounded-full bg-slate-50 border border-slate-200 flex items-center justify-center text-slate-400 hover:text-slate-900 transition-all">
                            <X className="w-5 h-5" />
                        </button>
                    </div>
                    ...
                    <div className="space-y-6">
                        <div className="space-y-3">
                            <label className="text-[10px] font-bold uppercase text-slate-400 tracking-[0.2em]">Kamera</label>
                            <select
                                value={selectedDevices.videoId}
                                onChange={(e) => onDeviceChange('videoId', e.target.value)}
                                className="w-full h-14 bg-slate-50 border border-slate-200 rounded-2xl px-4 text-sm font-bold text-slate-800 outline-none focus:ring-2 focus:ring-blue-500/20 transition-all appearance-none"
                            >
                        ...
                        <div className="space-y-3">
                            <label className="text-[10px] font-bold uppercase text-slate-400 tracking-[0.2em]">Mikrofon</label>
                            <select
                    ...
                    <button
                        onClick={onClose}
                        className="w-full py-5 rounded-2xl bg-[#1e3a8a] text-white text-[11px] font-bold uppercase tracking-widest shadow-xl shadow-blue-500/20 hover:bg-blue-900 transition-all"
                    >
                        Kaydet ve Kapat
                    </button>
                </div>
