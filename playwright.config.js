...
export default defineConfig({
    testDir: './tests/e2e',
    // *.auth.spec.js belongs to the mock-auth suite (config.auth.js).
    // *.real-auth.spec.js belongs to the emulator-backed suite
    // (config.emulator.js). Both run against bespoke builds; the
    // public/default suite must not pick them up.
    testIgnore: [/.*\.auth\.spec\.js$/, /.*\.real-auth\.spec\.js$/],
