...
7. **Çok katılımcılı mülakat wizard (4 adım)**: Adım 1 → Aday seç, Adım 2 → Katılımcı seç (isim/email arama, availability badge + freebusy API), Adım 3 → Zaman seç, Adım 4 → Onayla & Gönder (Google Calendar çoklu davet + email bildirim). Katılımcılar `session.participants[]` dizisinde `{ userId, name, email, role }` şemasıyla saklanır; `department_user` rolü "Benim Mülakatlarım" sekmesiyle yalnızca kendi mülakatlarını görür (cross-department: `participantInvites` koleksiyonu).
...
9. **Toplu CV Yükleme** (Task #10): `POST /api/bulk-import` — multipart/form-data ile max 100 PDF/DOCX dosyası kabul eder, Firestore'da `bulkImportJobs/{jobId}` iş kaydı oluşturur ve arka planda sıralı işler (kota hatalarında üstel geri çekilme). `GET /api/bulk-import/:jobId` — polling için durum endpoint. Frontend: `CandidateProcessPage.jsx` `onSnapshot` ile gerçek zamanlı ilerleme izler; iş tamamlanınca sağ altta toast bildirim gösterir. Firestore şema: `bulkImportJobs/{jobId}` → `{status, totalCount, processedCount, failedCount, positionId, positionTitle, avgScore, createdAt, completedAt}`; alt koleksiyon `items/{index}` → `{index, originalName, tempPath, status, matchScore, error}`.
