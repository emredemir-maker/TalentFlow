...
> **Sürüm**: 2.1 · **Güncelleme**: Mart 2026  
...
10. [Entegrasyon Merkezi](#10-entegrasyon-merkezi)  
11. [Deploy & Ortam](#11-deploy--ortam)
...
Google Workspace API köprüsü — **hibrit OAuth modu**:
- `connectGoogleWorkspace(userId, forceConsent?)` — Bağlantı akışı (aşağıya bakın)
- `ensureValidGoogleToken(userId, userProfile)` — Süresi dolan token yenileme
- `sendDirectEmail(userId, token, emailData)` — Gmail API ile mail gönderme
- `createDirectCalendarEvent(userId, token, eventData)` — Calendar etkinliği + Meet linki
- `getCalendarEvents(token, timeMin, timeMax)` — Müsaitlik kontrolü
- `fetchEmailThread(token, threadId)` — Mail konuşma geçmişi
- `checkGmailMessages(token, query)` — Gmail arama ve içerik okuma
- `disconnectGoogleWorkspace(userId)` — Bağlantı kesme

### `microsoftIntegrationService.js`
Microsoft 365 API köprüsü (admin credentials zorunlu):
- `connectMicrosoft(userId, idToken)` — Server-side OAuth popup akışı
- `ensureValidMicrosoftToken(userId, userProfile, idToken)` — Refresh token ile yenileme
- `sendMicrosoftEmail(token, emailData)` — Outlook API ile mail
- `createMicrosoftCalendarEvent(token, eventData)` — Teams toplantı linki dahil etkinlik
- `disconnectMicrosoft(userId)` — Bağlantı kesme
...
## 10. Entegrasyon Merkezi

> **Erişim**: Yalnızca `super_admin` rolündeki kullanıcılar — Sidebar'da "Geliştirici" grubunda listelenir, route düzeyinde de korunur (`App.jsx`: `super_admin` değilse `<Dashboard />` render edilir).
...
Admin (super_admin)
  │
  ├─ Entegrasyon Merkezi (IntegrationsPage)
  │    ├─ POST /api/admin/integrations  →  Firestore settings/integrations
  │    └─ GET  /api/admin/integrations  ←  Firestore + in-memory cache
  │
  └─ In-memory cache: integrationConfigs = { google: null, microsoft365: null }
       (server başlangıcında Firestore'dan yüklenir)

Kullanıcı (recruiter / department_user / super_admin)
  │
  └─ Ayarlar → Hesabım  →  connectGoogle / connectMicrosoft
...
Aynı `connectGoogleWorkspace()` çağrısı iki farklı akışa yönlenir:

| Durum | Akış | Kullanım Senaryosu |
|---|---|---|
| Admin özel credentials **girmemiş** | Firebase `signInWithPopup` | Mevcut test/geliştirme ortamı |
| Admin özel credentials **girmiş** | Server-side OAuth popup (`/api/auth/google/url`) | Yeni müşteriye deploy |

**Neden hibrit?**  
Firebase `signInWithPopup` hemen çalışır — yapılandırma gerekmez. Ancak uygulamayı farklı bir Firebase projesine veya farklı bir kuruluşa deploy ederken Firebase projesinden bağımsız, kalıcı `refresh_token` içeren bir akış gerekir. Admin Entegrasyon Merkezi'nden kendi Google Cloud credentials'larını girdiğinde sistem otomatik olarak server-side akışa geçer.

**Server endpoint'leri:**
```
GET  /api/auth/google/url?userId=&force=      → OAuth authorization URL üretir
POST /api/auth/google/exchange                → code → access_token + refresh_token
POST /api/auth/google/refresh                 → refresh_token → yeni access_token
```

**Callback sayfası:** `/auth/google/callback` → `GoogleCallbackPage.jsx`  
Popup içinde açılır, `postMessage({ type: 'GOOGLE_OAUTH_CALLBACK', email })` ile ana pencereye bildirir.

**Token saklama (Firestore):**
```js
users/{userId}.integrations.google = {
  connected: true,
  email: 'kullanici@ornek.com',
  accessToken: '...',
  refreshToken: '...',        // yalnızca server-side akışta
  tokenExpiresAt: 1234567890, // ms cinsinden
  connectedAt: '2026-03-29T...',
  authMode: 'firebase'        // yoksa server-side
}
```

**Kullanılan OAuth Scope'lar:**
...
**Mod: Tamamen admin yapılandırmalı** (Azure AD app registration zorunlu)

Admin Entegrasyon Merkezi'nden şu bilgiler girilmelidir:
...
**Redirect URI** (Azure'a eklenecek):  
`{uygulama-domain}/auth/microsoft/callback` — sayfa otomatik hesaplar, kopyalanabilir.

**Server endpoint'leri:**
```
GET  /api/auth/microsoft/url?userId=&force=   → Microsoft OAuth URL üretir
POST /api/auth/microsoft/exchange             → code → access_token + refresh_token
POST /api/auth/microsoft/refresh              → refresh_token → yeni access_token
```

**Callback sayfası:** `/auth/microsoft/callback` → `MicrosoftCallbackPage.jsx`

**Kullanılan OAuth Scope'lar:**
- `openid profile email offline_access`
- `User.Read Mail.Send Calendars.ReadWrite OnlineMeetings.ReadWrite`

---

### Entegrasyon Konfigürasyonu Firestore Yolu

```
artifacts/talent-flow/public/data/settings/integrations
  ├─ google: { clientId, clientSecret, redirectUri, configuredAt, configuredBy }
  └─ microsoft365: { tenantId, clientId, clientSecret, redirectUri, configuredAt, configuredBy }
```

Sunucu başlangıcında bu döküman `loadIntegrationConfigs()` ile okunur ve `integrationConfigs` in-memory cache'ine yüklenir. Admin kaydettiğinde hem Firestore güncellenir hem de cache anında güncellenir.
