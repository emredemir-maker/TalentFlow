            ...
            <div className="min-h-screen bg-[#0A0F1E] flex items-center justify-center p-6">
                <div className="w-full max-w-lg space-y-6 animate-in fade-in duration-500">
                    ...
                    <div className="bg-white/5 border border-white/10 rounded-3xl p-8 space-y-6">
                        ...
                        <button
                            onClick={handleStart}
                            className="w-full py-3 rounded-xl bg-cyan-500 hover:bg-cyan-400 text-white font-black text-sm transition-all flex items-center justify-center gap-2 shadow-lg shadow-cyan-500/20"
                        >
            ...
            <header className="h-14 bg-[#0D1426] border-b border-white/5 flex items-center justify-between px-5 shrink-0">
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div className="flex items-center gap-2 bg-red-500/10 border border-red-500/20 rounded-full px-3 py-1">
                        ...
                        <span className="text-[10px] font-bold text-red-400">{isRecording ? 'KAYIT' : 'DURDURULDU'}</span>
                    </div>
                    {/* Elapsed */}
                        ...
                        <div ref={transcriptRef} className="flex-1 overflow-y-auto px-5 py-3 space-y-3">
                            {transcript.length === 0 ? (
                                ...
                                <div key={i} className="flex gap-2.5 group">
                                    ...
                                    <p className="text-[11px] text-white/80 leading-relaxed flex-1">{entry.text}</p>
                                </div>
                            ))}
                        </div>
                    </div>
                ...
                <div className="w-[380px] shrink-0 flex flex-col overflow-hidden bg-[#0B1120]">
                            ...
                            {STRATEGIES.map(s => {
                                ...
                                return (
                                    <button
                                        ...
                                        className={`flex-1 flex items-center justify-center gap-1 py-1.5 rounded-lg text-[10px] font-bold transition-all ${activeStrategy === s.id ? 'bg-blue-600 text-white' : 'bg-white/5 text-white/30 hover:text-white hover:bg-white/10'}`}
                                    >
                    ...
                    <div className="flex-1 overflow-y-auto custom-scrollbar">
                        ...
                        <div className="px-4 pt-4 pb-3">
                            ...
                            {pathLoading ? (
                                ...
                                <div className="bg-blue-600/10 border border-blue-500/30 rounded-xl p-3.5 space-y-2">
                                    ...
                                    <p className="text-[12px] font-bold text-white leading-snug italic">
                                ...
                                <div className="space-y-1">
                                    {questions.map((q, i) => (
                                        ...
                                        >
                                            ...
                                            <span className={`text-[10px] leading-snug line-clamp-2 ${i === currentQIndex ? 'text-white/90' : 'text-white/40'}`}>{q.text}</span>
                                        </button>
                                    ))}
                                </div>
                            </div>
                        ...
                        <div className="px-4 pb-3 border-t border-white/5 pt-3">
                            ...
                            {aiInsights.length === 0 ? (
                                <p className="text-[10px] text-white/20 italic">Transkript uzadıkça tespitler burada görünecek…</p>
                            ) : (
                                <div className="space-y-2">
                                    {aiInsights.slice(0, 4).map(ins => (
                                        <div key={ins.id} className={`rounded-lg p-2.5 border text-[11px] leading-relaxed ${ins.type === 'warning' ? 'bg-yellow-500/10 border-yellow-500/25 text-yellow-200' : 'bg-white/5 border-white/10 text-white/85'}`}>
                                            {ins.text}
                                            {ins.hint && <p className="mt-1.5 text-cyan-300 italic text-[10px] font-medium">→ {ins.hint}</p>}
                                        </div>
                                    ))}
                                </div>
                            )}
                                ...
                                {STAR_KEYS.map(({ key, label }) => (
                                    <div key={key}>
                                        <div className="flex justify-between mb-1">
                                            ...
                                            <span className="text-[10px] font-mono font-bold text-white/60">{starScores[key]}</span>
                                        </div>
