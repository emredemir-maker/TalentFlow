...
export default function RequirementNormalizeModal({
    ...
    return (
        <div className="fixed inset-0 z-[120] flex items-center justify-center bg-n900/40 p-3">
            <div className="w-full max-w-2xl max-h-[85vh] flex flex-col rounded-[14px] bg-n0 border border-n200 shadow-sm overflow-hidden">
                <header className="flex items-center justify-between gap-2 px-4 py-3 border-b border-n100 shrink-0">
                    <h2 className="flex items-center gap-1.5 text-[10px] font-semibold text-n700 uppercase tracking-[0.08em]">
                        <Wand2 className="w-3.5 h-3.5 text-brand" /> Maddelere Ayır — Önizleme
                    </h2>
                    <button type="button" onClick={onCancel} className="p-1.5 rounded-lg hover:bg-n100 text-n400">
                        <X className="w-4 h-4" />
                    </button>
                </header>
                ...
                <div className="flex-1 overflow-y-auto px-4 py-3 space-y-3">
                    {loading && (
                        <p className="text-[10px] text-n500">Maddeler ayrıştırılıyor…</p>
                    )}
                                ...
                                {items.map((it, i) => (
                                    <div
                                        ...
                                        className={`flex items-start gap-2 rounded-lg border px-2.5 py-2 transition-colors ${
                                            dropped.has(i) ? 'border-n100 bg-n50 opacity-50' : 'border-n200'
                                        }`}
                                    >
                                        <label className="flex items-start gap-2 min-w-0 flex-1 cursor-pointer">
                                            ...
                                            />
                                            <span className="text-[10px] text-n700 leading-relaxed">{it.text}</span>
                                        </label>
                            ...
                            {proposal?.notes?.length > 0 && (
                                <ul className="space-y-0.5 pt-1">
                                    {proposal.notes.map((n) => (
                                        <li key={n} className="flex items-start gap-1 text-[10px] text-n500">
                                            <ArrowRight className="w-2.5 h-2.5 shrink-0 mt-0.5 text-n300" />
                                            {n}
                                        </li>
                                    ))}
                                </ul>
                ...
                <footer className="flex items-center justify-end gap-2 px-4 py-3 border-t border-n100 shrink-0">
                    <button
                        type="button"
                        onClick={onCancel}
                        className="px-3 py-1.5 rounded-lg border border-n200 text-[10px] font-semibold text-n500 uppercase tracking-wider hover:bg-n50"
                    >
                        Vazgeç
                    </button>
                    <button
                        ...
                        className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-brand hover:bg-brand-600 text-white text-[10px] font-semibold uppercase tracking-wider disabled:opacity-40"
                    >
...
function Alert({ tone, title, children }) {
    const cls = tone === 'red'
        ? 'border-transparent bg-bad-bg text-n700'
        : 'border-transparent bg-warn-bg text-n700';
    return (
