...
// Loaded from Firestore on startup; updated via POST /api/admin/integrations
...
async function handleDirectUrl(url, res, isVisual = false) {
    ...
    try {
        ...
        const candidate = await parseProfile(bodyText, 'gemini-2.5-flash');
...
// GET /api/auth/microsoft/url — generate OAuth authorization URL
...
// POST /api/auth/microsoft/exchange — exchange authorization code for tokens
app.post('/api/auth/microsoft/exchange', verifyFirebaseToken, async (req, res) => {
    try {
        ...
        if (!tokenRes.ok || tokenData.error) {
            console.error('[microsoft/exchange] Token error:', tokenData);
        ...
        console.log(`[microsoft/exchange] User ${userId} connected Microsoft account: ${email}`);
...
// POST /api/auth/microsoft/refresh — refresh an expired access token
...
app.post('/api/auth/google/exchange', verifyFirebaseToken, async (req, res) => {
    try {
        ...
        const tokenData = await tokenRes.json();
        if (!tokenRes.ok || tokenData.error) {
            console.error('[google/exchange] Token error:', tokenData);
            return res.status(400).json({ error: tokenData.error_description || 'Token değişimi başarısız.' });
        }
        ...
        console.log(`[google/exchange] User ${userId} connected Google account: ${email}`);
        res.json({ success: true, email });
    } catch (err) {
...
// POST /api/auth/google/refresh — refresh an expired access token
...
app.post('/api/ai/generate', aiLimiter, async (req, res) => {
    ...
    // Default JSON for backward compat; callers requesting free-form Turkish
    // text (feedback paragraphs, summaries, etc.) should pass 'text/plain'.
